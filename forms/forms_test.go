...
func testForm() *Form {
	...
	form.WithField("DOB", new(FieldBuilder).
		...
		if !value.(time.Time).Before(time.Now().UTC().AddDate(-18, 0, 1)) {
...
func TestValidForm(t *testing.T) {
	...
	formValues.Add("DOB", time.Now().UTC().AddDate(-18, 0, 0).Format(time.RFC3339))
...
func TestInvalidForm(t *testing.T) {
	...
	formValues.Add("DOB", time.Now().UTC().AddDate(-17, 0, 0).Format(time.RFC3339))
